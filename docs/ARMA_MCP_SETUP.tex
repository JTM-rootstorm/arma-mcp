...
\newcommand{\projectroot}{\code{/home/mike/Documents/Programming/arma-mcp}}
...
codex mcp add arma-mcp \
  --env ARMA_MCP_TOKEN=replace-with-a-local-random-token \
  -- node /home/mike/Documents/Programming/arma-mcp/sidecar/dist/index.js
\end{commandblock}
...
\begin{commandblock}
cd /home/mike/Documents/Programming/arma-mcp
...
\begin{itemize}
  \item \code{arma\_ping}: check sidecar, bridge, Eden connection, and pending commands.
  \item \code{arma\_request\_editor\_snapshot}: queue a selection or all-objects snapshot request.
  \item \code{arma\_get\_editor\_snapshot}: return the latest stored Eden snapshot.
  \item \code{arma\_generate\_checkpoint\_plan}: generate a dry-run checkpoint plan.
  \item \code{arma\_queue\_apply\_plan}: queue a reviewed structured plan for Eden.
  \item \code{arma\_get\_bridge\_events}: inspect recent bridge events and Eden results.
\end{itemize}
...
\begin{itemize}
  ...
  \item No asset index, mesh parsing, collision validation, road detection, terrain
        sampling, or faction-aware object catalog exists yet.
  \item Marker creation still needs live Eden verification.
  \item BattleEye/public-server deployment, addon signing, audit logs, and admin
...
This runs sidecar validation, extension build, HEMTT build, and git proof commands.
