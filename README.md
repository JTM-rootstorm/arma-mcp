...
5. Use `arma.ping`, `arma.bridge.diagnostics`, `arma.bridge.get_status`, `arma.bridge.get_capabilities`, and the Eden read/write tools to inspect, dry-run, apply, and re-read changes. If a Codex-managed session omits individual tool names, use `arma_discovery` to list fallback coverage and `arma_call` with an allowlisted `toolName`.
